		...
		<div class="export">
			...
			<form method="POST" name="export" action="edit.php?post_type=da_image&page=import_export">
				...
				foreach ( $da_images->posts as $key => $da_image ) :
					?>
					<input type="checkbox" name="export_ids[]" value="<?php echo $da_image->ID; ?>" id="export_id_<?php echo $da_image->ID; ?>" 
																				<?php
																				if ( in_array( $da_image->ID, $export_ids ) ) {
																					echo 'checked="checked"';}
																				?>
					/> <label for="export_id_<?php echo $da_image->ID; ?>"><?php echo $da_image->post_title; ?></label><br />
				<?php endforeach; ?>
