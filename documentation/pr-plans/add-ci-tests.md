...
- [x] A direct-access sweep runs in CI and fails when any shipped own-code PHP file executed directly produces output or a fatal (the unguarded-template class: master's templates leak fatal errors with server paths on direct HTTP access).
